...
public class MainActivity extends Activity implements OnItemSelectedListener {
	...
	public void fire(View v) {
		...
		if (selectedTarget.equals("NONE")) {
			Toast.makeText(this, "타겟이 설정되지 않았습니다.", Toast.LENGTH_SHORT).show();
		} else {
						...
						while (ServiceDenier.firing) {
							...
							case 0:
								MainActivity.this.runOnUiThread(new Runnable() {
									public void run() {
										tvTime.setText("소모 시간: " + (System.currentTimeMillis() - TCPSocketThread.startTime) / 1000.0 + "초");
										tvHits.setText("TCP Hits: " + TCPSocketThread.count);
										tvPacketsPerSec.setText("초당 패킷: " + Math.round(TCPSocketThread.count / ((System.currentTimeMillis() - TCPSocketThread.startTime) / 1000.0)));
									}
								});
								break;
							case 1:
								MainActivity.this.runOnUiThread(new Runnable() {
									public void run() {
										tvTime.setText("소모 시간: " + (System.currentTimeMillis() - UDPSocketThread.startTime) / 1000.0 + "초");
										tvHits.setText("UDP Hits: " + UDPSocketThread.count);
										tvPacketsPerSec.setText("초당 패킷: " + Math.round(UDPSocketThread.count / ((System.currentTimeMillis() - UDPSocketThread.startTime) / 1000.0)));
									}
								});
								break;
							case 2:
								MainActivity.this.runOnUiThread(new Runnable() {
									public void run() {
										tvTime.setText("소모 시간: " + (System.currentTimeMillis() - HTTPSocketThread.startTime) / 1000.0 + "초");
										tvHits.setText("HTTP Hits: " + HTTPSocketThread.count);
										tvPacketsPerSec.setText("초당 패킷: " + Math.round(HTTPSocketThread.count / ((System.currentTimeMillis() - HTTPSocketThread.startTime) / 1000.0)));
									}
								});
								break;
							}
							...
							if(ServiceDenier.error) {
								serviceDenier.stop();
								tvTime.setText("소모시간: " + (System.currentTimeMillis() - TCPSocketThread.startTime) / 1000.0 + "초");
								tvHits.setText("Exception Occurred");
								tvPacketsPerSec.setText("Error - 상세내용은 로그를 확인해 주세요");
								b.setText("공격");
							}
